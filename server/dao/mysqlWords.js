...
export function addWord(word, group) {
  return new Promise((resolve, reject) => {
    if (!connectionPool) {
      connectionPool = mysql.createPool(dbConfig);
    }
    connectionPool.query('INSERT INTO words VALUES (?, ?)', [word, group], function (error, results, fields) {
      if (error) {
        reject(error);
      } else {
        resolve(results);
    ...
    if (connectionPool) {
      connectionPool.end(err => {
        if (err) {
          ...
          resolve('pool is ended');
        }
      })
    } else {
      resolve('pool is not created');
    }
  })
}
